...
<body class="gray-bg">
				...
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStudent.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>所选课程：
								<select class="form-control" name="course_id" id = "course_id" aria-required="true" required>
									<option value="">---请选择---</option>
	...
	<!-- 新增和修改对话框 -->
	<div class="modal fade" id="modal_course" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_addCourse" method="post" action="addCourse.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>课程名：<input type="text"  name="name"
										   class="form-control" aria-required="true" required/></td>
								<td>课程编码：<input type="text"  name="code"
										   class="form-control" aria-required="true" required/></td>
							</tr>
						</table>
						<div class="modal-footer">
	...
	var TableInit = function () {
	    ...
	    oTableInit.Init = function () {
	        $('#table_user').bootstrapTable({
	            url: 'jiyunstudentList.htm',         //请求后台的URL（*）
	            method: 'post',                      //请求方式（*）
	           ...
	           // height: 500,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	            ...
	            },
	            {
					...
					},
					{
						...
						sortable:true,
						formatter:function(value,row,index){
							// return new Date(row.entrytime).Format("yyyy-MM-dd HH:mm:ss");
	...
	function timeform(time) {
		var d=new Date(time);
		var n=d.getFullYear();
		var y=(d.getMonth()+1)<10?'0'+(d.getMonth()+1):(d.getMonth()+1);
		var r=d.getDay()<10?'0'+d.getDay():d.getDay();
		return n+"-"+y+"-"+r;
	}
	function initEditUser(getSelection){
		...
		$('#course_id').val(getSelection.course_id);
		var date=new Date(getSelection.entrytime);
		$('#entrytime').val(timeform(getSelection.entrytime));
